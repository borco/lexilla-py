...
Both files embed the same project badges, each wrapped in matching
`<!-- sync:NAME -->`/`<!-- /sync:NAME -->` comments. This script extracts
every named block from both files and fails if a block is missing from
either file, or if a pair of blocks differs.
"""
